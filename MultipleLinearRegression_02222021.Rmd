...
str(homes) # 781 observations, 7 columns/variables

# NEW DATASET <- OLD DATASET
homes_sub <- homes %>% # PIPE, AND THEN WE DO THIS
  filter(City == "Arroyo Grande" | City == "San Luis Obispo" | City == "Atascadero" | City == "Santa Maria-Orcutt") 

dim(homes_sub) #403 observations, 7 variables
str(homes_sub)
summary(homes_sub)
...
linear regression: **y** = intercept (beta0) + slope of **X** (beta1X1) + error
  plant height = intercept + sunlight + error
...
multiple linear regression: **y** = intercept + slope of **X1** (beta1X1) + slope of **X2**(beta2X2) + slope of **X3**(beta3X3) + error
  plant height = intercept + sunlight + species + precipitation 
...
by_city <- ggplot(homes_sub, aes(x = Price)) + 
  ...
  theme_bw() + # there's multiple themes, show theme_bw
...
# child height ~ mom's height + nutrition (2x)
# child height ~ mom's height + mom's jean length, this is high collinearity, avoid this (2X) --> 1X

pairs.panels(homes_sub, lm =TRUE)
dim(homes_sub)
...
# 0 flat line, there is no relationship 
```
price ~ city
...
pairs.panels(homes_sub, cor = TRUE)
...
# p-value: 
  # tells you if your model is a significant predictor for slopes of Xs, for this model p = < 2.2e-16 (significant!)

# F-statistic: 
  #your F crit, did added coefficients improve model, for this model F = 245.9

# Intercept: 
  #this is your beta0, for this model beta0 = -626690.88

# y = mx + b
# Estimates: 
  #describes relationship between predictor variable & response, for this model looking at 'SqFt' estimate = 230.73

# Multple R-square: 
  #how much variation can be described with your model, for this model Multiple R^2 = 0.8492
...
# what is the 
  ...
  # what if your p-pvalue? #  p-value: < 2.2e-16
  # what is your F-statistic?
  # what is your intercept (beta0)? # 106333.12
  # what is an estimate for a strong predictor variable?
  # what is your multiple R^2? # 0.5339 --> 53% of variation is explained by model homes_lm2
```
